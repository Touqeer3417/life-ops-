...
class GoogleIntegrationService:
    """
    Own the Google OAuth lifecycle for a LifeOps user.

    Auth0 authenticates the LifeOps user. Google OAuth is kept
    separate and only grants that authenticated LifeOps user
    access to their Google services.
    """
    ...
    async def create_authorization_url(
        ...
        await self.repository.set_pending_authorization(
            connection,
    ...
    async def complete_authorization(
        ...
        *,
        ...
        if (
            ...
            await self.repository.clear_pending_authorization(
                connection
            )
        ...
        # This makes a callback state single-use even
        # if token exchange later fails.
        await self.repository.clear_pending_authorization(
            connection
        )
        ...
        refresh_token_encrypted = (
            self.token_cipher.encrypt_optional(
        ...
        if (
            ...
            await self.repository.mark_reauthorization_required(
                connection,
                error_code=(
                    "refresh_token_missing"
                ),
                error_message=(
                    "Google did not provide an "
        ...
        await self.repository.mark_connected(
            connection,
    ...
    async def disconnect(
        ...
        if revocation_token:
            try:
                ...
                # Local disconnect must still succeed.
                # Keeping credentials after a user asks
                # to disconnect is worse than failing
                # closed because Google's revoke service
        ...
        await self.repository.mark_disconnected(
            connection,
            disconnected_at=(
                self._utc_now()
            ),
    ...
    async def get_valid_access_token(
        ...
        *,
        ...
        if connection is None:
            raise OAuthConnectionRequiredError()

        if connection.requires_reauthorization:
            raise (
                OAuthReauthorizationRequiredError()
            )
    ...
    async def _refresh_access_token(
        ...
        await self.repository.update_refreshed_tokens(
            connection,
            access_token_encrypted=(
                self.token_cipher.encrypt(
                    token_response.access_token
    ...
    def _scopes_for_access_level(
        ...
        if self._has_scope(
            ...
            # Never downgrade an already granted
            # Calendar write connection simply
            # because the user starts a read flow.
            existing.add(
                GOOGLE_CALENDAR_READ_SCOPES[
                    1
                ]
        ...
        if missing:
            raise (
                OAuthInsufficientScopeError(
                    ...
                    "permission(s). Reconnect "
                    "Google with the required "
                    "access level."
                )
            )
