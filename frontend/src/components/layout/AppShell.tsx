import { LayoutDashboard, LogOut, Settings2, ShieldCheck, Sparkles, UserRound } from 'lucide-react'
...
const navItems = [
  ...
  { to: '/app/profile', label: 'Profile & preferences', icon: Settings2, end: false },
]

export function AppShell() {
  const { logout, identity } = useAuth()
  const currentUser = useCurrentUser()
  const displayName = currentUser.data?.full_name ?? identity?.name ?? 'LifeOps user'
  const email = currentUser.data?.email ?? identity?.email ?? ''
        ...
        <aside className="hidden w-72 shrink-0 border-r border-slate-200 bg-white p-5 lg:flex lg:flex-col">
          ...
          <div className="mt-auto rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-slate-500">
              Auth0 access tokens are validated by FastAPI before protected user data is returned.
            </p>
          </div>
        </aside>
          ...
          <div className="border-b border-slate-200 bg-white px-4 py-2 lg:hidden">
            <nav className="flex gap-2 overflow-x-auto">
              <NavLink
                ...
                className={({ isActive }) => cn('rounded-lg px-3 py-2 text-sm font-medium', isActive ? 'bg-slate-950 text-white' : 'text-slate-600')}
              >
                Dashboard
              </NavLink>
              <NavLink
